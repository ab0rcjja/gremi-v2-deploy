...
const aiCall = async (system, userMsg, maxTok=1500) => {
  try {
    ...
    if(d.error) { console.error("AI error:", d.error); return "Error: "+d.error.message; }
    return d.content?.[0]?.text || "";
  } catch(e) { console.error("aiCall exception:", e); return "Error: "+e.message; }
}
...
Top deals by workers: ${active.filter(l=>parseInt(l.workers)>0).sort((a,b)=>(parseInt(b.workers)||0)-(parseInt(a.workers)||0)).slice(0,3).map(l=>`${l.company} ${l.workers}w ${l.stage}`).join("; ")}`;
  ...
  return (
    <div style={{flex:1,overflowY:"auto",padding:12,paddingBottom:72,display:"flex",flexDirection:"column",gap:10}}>
      ...
      <ActionCard icon="⚠" title="Overdue" color={C.red} items={overdue}/>
...
LANGUAGE: Respond in the same language the user writes in.`;
  ...
  const send = async (userText) => {
    ...
    try {
      ...
      if(!res.ok){const e=await res.text();throw new Error("HTTP "+res.status+": "+e.substring(0,100));}
      const d = await res.json();
      const raw = d.content?.[0]?.text||(d.error?"Error: "+d.error.message:"Error.");
...
function DashboardTab({locs, hqs, users, cur, onSelectLoc, isAdmin, isTeamLead}) {
  ...
  const sendBriefMsg=async()=>{
    ...
    const t=await aiCall("You are the sales AI assistant for Gremi Personal Romania. Respond concisely. IMPORTANT: Reply in the SAME LANGUAGE the user writes in — Polish → Polish, Romanian → Romanian, Russian → Russian, English → English. If they ask for a plan update, propose a new version.",ctx+"\n\nUser: "+txt,500);
...
- IMPORTANT: respond in ${({en:'English',pl:'Polish / polski',ro:'Romanian / română',ru:'Russian / русский'})[briefLang]||'English'}`;
      ...
      <div style={{flex:1,overflowY:"auto",padding:12,paddingBottom:70,display:"flex",flexDirection:"column",gap:10}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          ...
          <div style={{background:`linear-gradient(135deg,${C.bg2},${C.bg3})`,border:`1px solid ${C.teal}44`,borderRadius:12,padding:14}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:(summaryLoading||summary||aiLoading||aiAnalysis)?10:0}}>
              ...
              {section==="actions"&&(
                <div style={{display:"flex",gap:2,background:C.bg3,borderRadius:7,border:`1px solid ${C.border}`,padding:"2px 3px",flexShrink:0}}>
                  {BRIEF_LANGS.map(l=>(
                    <button key={l.id} className="btn" onClick={()=>{setBriefLangSave(l.id);setSummary("");}}
                      title={l.label}
                      style={{padding:"3px 7px",fontSize:11,borderRadius:5,border:"none",
                        background:briefLang===l.id?`${C.teal}25`:"transparent",
                        color:briefLang===l.id?C.teal:C.txt3,fontWeight:briefLang===l.id?700:400}}>
                      {l.flag}
                    </button>
                  ))}
                </div>
        ...
        {section==="actions"&&(
          <>
            ...
            <ActionGroup icon="⚠" title="Overdue" color={C.red} items={overdue}/>
